...
//! its layers. Detecting a DFlash GGUF, finding one beside the model, the
//! drafter context (a default context bound to the target), feature
//! injection into the drafter's cache, and the draft/verify rounds with
//! their adaptive draft length.
...
/// GGUF, else one discovered beside the model. DSpark drafters are not
/// run: they are skipped with a warning.
#[must_use]
pub fn resolve_drafter(configured: Option<&str>, model_path: &str) -> Option<String> {
    let usable = |path: &str| {
        if !model_is_dflash(path) {
            return false;
        }
        if model_is_dspark(path) {
            tracing::warn!(
                path,
                "DSpark drafters are not supported; continuing without this drafter"
            );
            return false;
        }
        true
    ...
    configured
        ...
        .or_else(|| discover_external_dflash(model_path).filter(|path| usable(path)))
}
...
/// The draft length a run starts with: at least one, at most one less than
/// the drafter's block, and small enough that the target's verification
/// batch (the anchor plus the drafts) fits its `n_batch`. `None` when the
/// target batch cannot hold even one draft.
fn clamp_draft_n(draft_n: usize, block_size: usize, target_batch: usize) -> Option<usize> {
...
impl<'m> DflashRuntime<'m> {
    /// A drafter context bound to `target_ctx`. Fails, so the run goes on
    /// without DFlash, for a drafter whose target layers the target lacks,
    /// whose feature width or vocabulary does not match the target, without
    /// a mask token inside its vocabulary, whose DFlash2 selector does not
    /// fit its output rows, or when the target batch cannot verify a draft.
    pub fn new(
        ...
        let draft_n = clamp_draft_n(draft_n, block_size, target_batch).ok_or_else(|| {
            DflashError(format!(
    ...
    pub fn refresh_settings(&mut self, p_min: f32) {
        self.p_min = p_min.clamp(0.0, 1.0);
    }
    ...
    /// Decodes `[anchor, mask * steps]` from `anchor_pos` in the drafter,
    /// reads the mask rows (greedy top-10 tokens, or the DFlash2 selector
    /// walk) until one falls below `p_min`, and drops the block from the
    /// drafter's cache again.
    fn draft_block(
        &mut self,
        anchor: LlamaToken,
        anchor_pos: i32,
        ...
        let drafted = if selector {
            ...
            for i in 1..n_block {
                let Some((token, prob)) =
                    greedy_token_with_prob(self.draft.get_logits_ith(i as i32))
                else {
                    break;
                };
                if prob < self.p_min {
...
mod tests {
    ...
    fn the_draft_length_fits_inside_the_block_and_the_target_batch() {
        assert_eq!(clamp_draft_n(4, 16, 512), Some(4));
        assert_eq!(clamp_draft_n(15, 8, 512), Some(7));
        assert_eq!(clamp_draft_n(0, 16, 512), Some(1));
        assert_eq!(clamp_draft_n(5, 1, 512), Some(1));
        assert_eq!(clamp_draft_n(15, 16, 8), Some(7));
        assert_eq!(clamp_draft_n(15, 16, 2), Some(1));
        assert_eq!(clamp_draft_n(4, 16, 1), None);
        assert_eq!(clamp_draft_n(4, 16, 0), None);
    ...
    fn drafters_are_found_by_their_gguf_key_and_name() {
        ...
        assert_eq!(resolve_drafter(Some(dspark), model_path), Some(found));
        std::fs::remove_dir_all(&dir).ok();
    }
}
