...
pub(crate) async fn run<S: SecretStore + ?Sized>(
    ...
    let outcome = if streaming {
        let response =
            crate::streaming::streaming::await_cancelable(runtime, request.cancellation, async {
                network
                    .post_json_stream(
                        &endpoint,
                        &path,
            ...
            .await?;
        crate::streaming::streaming::consume_stream(
            response,
            crate::streaming::stream_framing::StreamFormat::Sse,
            crate::streaming::stream_normalize::StreamProtocol::OpenAi,
            runtime,
            &request,
        )
        .await?
...
fn parse_response(response: JsonResponse) -> Result<InferenceOutcome, AdapterError> {
    ...
    for (position, choice) in parsed.choices.into_iter().enumerate() {
        ...
        let raw_text = message
            ...
            .unwrap_or_default();
        let (text, tagged_reasoning) =
            crate::streaming::stream_normalize::split_complete_thinking(&raw_text);
        let explicit_reasoning = [message.reasoning, message.reasoning_content]
            .into_iter()
            .flatten()
            .filter_map(|value| value.as_str().map(str::to_owned))
            .collect::<Vec<_>>();
        let reasoning = crate::streaming::stream_normalize::merge_complete_reasoning(
            tagged_reasoning,
...
mod tests {
    ...
    fn provider_tool_choice_policies_keep_their_legacy_wire_differences() {
        assert_eq!(
            crate::providers::mistral::Mistral
    ...
    fn legacy_usage_details_match_buffered_and_streamed_responses() {
        ...
        use crate::streaming::stream_normalize::{StreamNormalizer, StreamProtocol};
        let cases = [
            (
