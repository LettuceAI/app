...
pub(crate) async fn run<S: SecretStore + ?Sized>(
    ...
    if streaming {
        let response =
            ...
            .await?;
        let outcome = crate::streaming::streaming::consume_stream(
            response,
            crate::streaming::stream_framing::StreamFormat::Ndjson,
            crate::streaming::stream_normalize::StreamProtocol::Ollama,
            runtime,
            &request,
        )
        .await?;
...
fn parse_response(response: JsonResponse) -> Result<InferenceOutcome, AdapterError> {
    ...
    let (text, tagged_reasoning) = crate::streaming::stream_normalize::split_complete_thinking(
