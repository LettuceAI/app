...
use crate::providers::gemini_cache::{GeminiCache, PreparedCache};
...
pub(crate) async fn run<S: SecretStore + ?Sized>(
    ...
    if streaming {
        let stream_path = path
            ...
            .ok_or(AdapterError::Rejected)?;
        let mut response = crate::streaming::streaming::await_cancelable(
            runtime,
            request.cancellation,
            ...
            && response.status == 404
        {
            ...
            response = crate::streaming::streaming::await_cancelable(
                runtime,
                request.cancellation,
                send_generate_stream(
                    provider,
        ...
        let (outcome, replay) = crate::streaming::streaming::consume_stream_with_provider_replay(
            response,
            crate::streaming::stream_framing::StreamFormat::Sse,
            crate::streaming::stream_normalize::StreamProtocol::Gemini,
            runtime,
            &request,
        )
        .await?;
