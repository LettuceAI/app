...
use crate::{
    ...
    NewCreationRegeneration, NewCreationTurn, NewCreationTurnAttempt, NewCreationWorkflow,
};
...
pub trait CreationAttemptRepository: Send + Sync {
    ...
    /// Discards the latest answered turn and admits its regeneration.
    fn admit_creation_regeneration(
        &self,
        regeneration: NewCreationRegeneration,
    ) -> Result<CreationTurnAttemptAdmission, CreationRepositoryError>;
    ...
    /// oldest first, with the parts of that attempt. A turn regenerated by
    /// `before` or an earlier turn is left out.
