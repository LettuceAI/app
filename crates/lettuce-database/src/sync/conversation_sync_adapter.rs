...
use crate::conversation::conversation_history_writer as history;
...
pub(crate) fn sync_load_conversation_root(
    ...
    for message_id in initial_ids {
        if let Some(message) = crate::backup::backup_adapter::read_conversation_message(
            transaction,
            id,
...
pub(crate) fn sync_load_conversation_message(
    ...
    let Some(mut backup) = crate::backup::backup_adapter::read_conversation_message(
        transaction,
        conversation_id,
        message_id,
    )
    ...
    let mut turns = crate::backup::backup_adapter::read_candidate_turns(
        transaction,
        conversation_id,
        message_id,
    )
