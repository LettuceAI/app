...
use crate::{
    conversation::conversation_artifact_adapter, conversation::conversation_creator,
    conversation::conversation_mutation_kernel as kernel,
...
pub(crate) fn insert_historical_conversation(
    ...
    if let Some(companion) = input.companion {
        crate::conversation::state_adapter::create_in(
            transaction,
            companion.owner,
            &companion.initial,
            conversation.created_at,
        )
        .map_err(crate::conversation::state_adapter::conversation_state_error)?;
        crate::conversation::state_adapter::insert_continuity_episode_in(
            transaction,
            companion.owner,
            &companion.episode,
        )
    ...
    for attempt in attempts {
        ...
        if let Some(runtime) = evidence.runtime.get(&attempt.id) {
            if let Some(speaker) = &runtime.speaker_inference {
                ...
                crate::conversation::speaker_inference_adapter::insert_restored_in(
                    transaction,
                    speaker,
                )?;
            }
            if runtime.initial_inference.is_some() || !runtime.tools.is_empty() {
                ...
                if let Some(initial) = &runtime.initial_inference {
                    crate::conversation::initial_inference_adapter::insert_restored_in(
                        transaction,
                        initial,
                    )?;
                }
                for tool in &runtime.tools {
                    crate::conversation::tool_adapter::insert_restored_in(transaction, tool)?;
                }
            }
