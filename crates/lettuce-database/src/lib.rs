...
const MIGRATION_9: Migration = Migration {
    id: 9,
    sql: include_str!("../migrations/0009_memory.sql"),
};

const MIGRATION_10: Migration = Migration {
    id: 10,
    sql: include_str!("../migrations/0010_usage.sql"),
};

const MIGRATION_11: Migration = Migration {
    id: 11,
    sql: include_str!("../migrations/0011_creation.sql"),
};

const MIGRATION_12: Migration = Migration {
    id: 12,
    sql: include_str!("../migrations/0012_companion_soul.sql"),
};

const MIGRATION_13: Migration = Migration {
    id: 13,
    sql: include_str!("../migrations/0013_companion_state.sql"),
};

const MIGRATION_14: Migration = Migration {
    id: 14,
    sql: include_str!("../migrations/0014_speech.sql"),
};

const MIGRATION_15: Migration = Migration {
    id: 15,
    sql: include_str!("../migrations/0015_whisper_models.sql"),
};

const MIGRATION_16: Migration = Migration {
    id: 16,
...
const MIGRATION_18: Migration = Migration {
    id: 18,
...
const MIGRATION_20: Migration = Migration {
    id: 20,
    sql: include_str!("../migrations/0020_restore.sql"),
};

const MIGRATION_21: Migration = Migration {
    id: 21,
    sql: include_str!("../migrations/0021_local_llm.sql"),
};

const MIGRATION_22: Migration = Migration {
    id: 22,
...
impl GlobalSettingsStore for Database {
    ...
    fn set_dynamic_memory_model_profile(
        ...
        if current.dynamic_memory_model_profile_id == model_profile_id {
            return Ok(current);
        }
        let changed = self
            .connection()
...
mod tests {
    ...
    fn m8_file_has_final_runtime_contract_and_checksums() {
        let path = std::env::temp_dir().join(format!("lettuce-m8-final-{}.db", MediaBlobId::new()));
        {
            ...
            let transaction = connection
                .transaction_with_behavior(TransactionBehavior::Immediate)
                .expect("M8 fixture transaction");
            for (conversation_id, branch_id, author_note, author_provenance) in [
                (
                    "m8-settings-launch",
                    "m8-branch-launch",
                    None::<&str>,
                ...
                transaction
                    .execute(
                        "INSERT INTO conversations (id, kind, lifecycle, title, active_branch_id, kind_json, revision, created_at, updated_at) VALUES (?1, 'direct', 'active', 'M8 settings', ?2, '{\"format_version\":1,\"value\":null}', 1, 0, 0)",
                        params![conversation_id, branch_id],
                    )
                    .expect("M8 settings conversation");
                transaction
                    .execute(
                        "INSERT INTO conversation_branches (conversation_id, id, status, revision, created_at, updated_at) VALUES (?1, ?2, 'active', 1, 0, 0)",
                        params![conversation_id, branch_id],
                    )
                    .expect("M8 settings branch");
                if conversation_id == "m8-settings-launch" {
                    transaction
                        .execute(
                            "INSERT INTO conversation_participants (conversation_id, id, role, ordinal, source_kind, enabled, muted, display_name, model_selection_json, revision, created_at, updated_at) VALUES (?1, 'm8-user-participant', 'user', 0, 'user', 1, 0, 'M8 User', '{\"format_version\":1,\"value\":null}', 1, 0, 0)",
                            params![conversation_id],
                        )
                        .expect("M8 user participant");
                    transaction
                        .execute(
                            "INSERT INTO conversation_message_revisions (conversation_id, id, message_id, branch_id, sequence, parts_json, authored_at) VALUES (?1, 'm8-user-revision', 'm8-user-message', ?2, 1, '{\"format_version\":1,\"value\":null}', 0)",
                            params![conversation_id, branch_id],
                        )
                        .expect("M8 user revision");
                    transaction
                        .execute(
                            "INSERT INTO conversation_messages (conversation_id, id, branch_id, author_participant_id, role, timeline_ordinal, logical_time, effective_time, visibility, pinned, scene_edited, active_revision_id, revision, created_at, updated_at) VALUES (?1, 'm8-user-message', ?2, 'm8-user-participant', 'user', 1, 0, 0, 'visible', 0, 0, 'm8-user-revision', 1, 0, 0)",
                            params![conversation_id, branch_id],
                        )
            ...
            transaction.commit().expect("M8 settings fixture commit");
        }
        let database = Database::open(&path).expect("upgrade m8 file");
        let connection = database.connection().expect("database lock");
        assert_eq!(
            connection
                .query_row("SELECT count(*) FROM schema_migrations", [], |row| row
        ...
        for kind in ["'interrupt'", "'flags'", "'rename'", "'resolve_speaker'"] {
            assert!(
        ...
        let launch: (Option<String>, String, String, String) = connection
            ...
            .expect("read canonical launch settings");
        assert_eq!(
            launch,
            (
                None,
                "launch_inherited".into(),
                "launch_inherited".into(),
                "disabled".into()
            )
        );
        let blank: (Option<String>, String) = connection
            .query_row(
                "SELECT author_note, author_note_provenance FROM conversation_settings WHERE conversation_id = 'm8-settings-blank'",
                [],
                |row| Ok((row.get(0)?, row.get(1)?)),
            )
    ...
    fn lorebook_feature_parameters_round_trip_and_validate() {
        ...
        old.as_object_mut()
            .expect("object")
            .remove("feature_parameters");
        let decoded: ModelProfileConfig = serde_json::from_value(old).expect("old model config");
        assert_eq!(decoded.feature_parameters, Default::default());
        model.config.feature_parameters.lorebook_generator.parameters.temperature = Set(0.6);
        model.config.feature_parameters.lorebook_generator.parameters.max_output_tokens = Set(8192);
        ...
        invalid.config.feature_parameters.lorebook_generator.parameters.top_p = Set(2.0);
        assert_eq!(
    ...
    fn account_profile_and_default_obey_foreign_keys() {
        ...
        let settings = GlobalSettingsStore::load(&database).expect("reload settings");
        assert_eq!(
