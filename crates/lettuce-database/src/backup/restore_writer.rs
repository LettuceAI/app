...
use crate::Database;
use crate::lorebook::lorebook_adapter::OwnerKind;
...
impl ProviderBackupRestoreWriter for Database {
    fn restore_provider_backup_graph(
        ...
        for prompt in &graph.prompts {
            crate::catalog::prompt_adapter::insert_imported_document(&transaction, prompt)
                .map_err(invalid)?;
        }
        for provider in &graph.audio_providers {
            crate::media::tts_adapter::insert_audio_provider_row(&transaction, provider)
                .map_err(invalid)?;
        }
        for voice in &graph.user_voices {
            crate::media::tts_adapter::insert_user_voice_row(&transaction, voice)
                .map_err(invalid)?;
        }
        for persona in &authored.personas {
            crate::catalog::persona_adapter::insert_persona(&transaction, persona.clone())
                .map_err(invalid)?;
        }
        for lorebook in &authored.lorebooks {
            crate::lorebook::lorebook_adapter::insert_lorebook_details(&transaction, lorebook)
                .map_err(invalid)?;
        }
        for details in &authored.characters {
            crate::catalog::character_adapter::insert_character_plan(
                &transaction,
                &CreateCharacterPlan {
        ...
        for details in &authored.groups {
            crate::catalog::group_adapter::insert_group_rows(
                &transaction,
                &CreateGroupPlan {
        ...
        for owner in &authored.character_lorebooks {
            crate::lorebook::lorebook_adapter::insert_bindings_in(
                &transaction,
                OwnerKind::Character,
        ...
        crate::media::speech_learning_adapter::insert_learning_batch_in(
            &transaction,
        ...
        for space in &graph.memory.unbound_pools {
            ...
            transaction
                ...
                .map_err(invalid)?;
            crate::memory::memory_adapter::insert_items(&transaction, space.id, &space.items)
                .map_err(invalid)?;
            transaction
                .execute(
                ...
                .map_err(invalid)?;
            crate::conversation::conversation_history_writer::insert_projections_in(
                &transaction,
                space.id,
        ...
        for history in histories {
            ...
            crate::conversation::conversation_history_writer::insert_historical_conversation(
                ...
                crate::conversation::conversation_history_writer::HistoricalConversation {
                    ...
                    usage: &events,
                    snapshots: Vec::new(),
                    creation: crate::conversation::conversation_history_writer::HistoricalCreation::Exact {
                        operations,
                        events: events_out,
                    },
                    memory,
        ...
        for session in &companion.sessions {
            ...
            .map_err(invalid)?;
            crate::conversation::state_adapter::replace_signals(
                &transaction,
                owner.conversation_id,
                &session.emotional_state,
        ...
        for soul in &companion.souls {
            ...
            let scope = crate::companion::soul_adapter::scope(owner);
            ...
            transaction
                ...
                .map_err(invalid)?;
            crate::companion::soul_adapter::insert_facts(&transaction, owner, &soul.facts)
                .map_err(invalid)?;
        ...
        for note in &companion.scheduled_notes {
            crate::memory::scheduled_note_adapter::insert_restored_note_in(&transaction, note)
                .map_err(invalid)?;
        }
        ...
        for rewind in rewinds {
            crate::memory::dynamic_memory_rewind_adapter::insert_restored_rewind_in(
                &transaction,
                rewind,
            )
            .map_err(invalid)?;
        ...
        for run in &companion.growth_runs {
            crate::companion::growth_adapter::insert_restored_in(&transaction, run)
                .map_err(invalid)?;
        }
        for run in &companion.consolidation_runs {
            crate::memory::consolidation_adapter::insert_restored_in(&transaction, run)
                .map_err(invalid)?;
        }
        for run in &companion.soul_writer_runs {
            crate::companion::soul_writer_adapter::insert_restored_in(&transaction, run)
                .map_err(invalid)?;
        }
        let creation = &graph.creation;
        for workflow in &creation.workflows {
            crate::catalog::creation_adapter::insert_restored_workflow_in(&transaction, workflow)
                .map_err(invalid)?;
        }
        for entry in &creation.lorebook_entry_runs {
            crate::lorebook::lorebook_entry_run_adapter::insert_restored_in(
                &transaction,
                &entry.run,
                &entry.attempts,
            )
        ...
        for entry in &creation.lorebook_keyword_runs {
            crate::lorebook::lorebook_keyword_run_adapter::insert_restored_in(
                &transaction,
                &entry.run,
                &entry.attempts,
            )
        ...
        for run in &creation.staged_lorebook_writer_runs {
            crate::lorebook::staged_lorebook_writer_adapter::insert_restored_in(&transaction, run)
                .map_err(invalid)?;
        }
