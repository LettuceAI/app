...
impl CharacterFileRepository for Database {
    ...
    fn import_character_file(
        ...
        for lorebook in &import.lorebooks {
            crate::lorebook::lorebook_adapter::insert_lorebook_details(&tx, lorebook).map_err(
                |error| match error {
        ...
        crate::catalog::character_adapter::insert_character_plan(&tx, &import.character).map_err(
            |error| match error {
        ...
        for note in &import.scheduled_notes {
            crate::memory::scheduled_note_adapter::insert_note_in(&tx, note).map_err(|error| {
                match error {
        ...
        let details =
            crate::catalog::character_adapter::load_details(&tx, import.character.character.id)
                .map_err(|_| CharacterFileRepositoryError::Storage)?
...
fn insert_companion_memory(
    ...
    if let Some(pool) = &memory.pool {
        ...
        .map_err(storage)?;
        crate::memory::memory_adapter::insert_items(tx, pool.id, &pool.items)
            .map_err(|_| CharacterFileRepositoryError::InvalidInput)?;
    }
...
fn load_export_record(
    ...
    let Some(details) = crate::catalog::character_adapter::load_details(connection, id)? else {
        return Ok(None);
    };
    ...
    for lorebook_id in bindings.query_map([id.to_string()], |row| row.get::<_, String>(0))? {
        ...
        if let Some(lorebook) =
            crate::lorebook::lorebook_adapter::load_details(connection, lorebook_id)?
        {
...
fn load_companion_memory(
    ...
    let pool = match pool_id {
        Some(pool_id) => {
            ...
            crate::memory::memory_adapter::get_in(tx, pool_id).map_err(invalid)?
        }
        None => None,
    };
    let soul_facts = crate::companion::soul_adapter::get_in(tx, SoulOwner::Character(id))
        .map_err(invalid)?
...
fn load_references(connection: &Connection) -> rusqlite::Result<CharacterFileReferences> {
    ...
    for row in prompts.query_map([], |row| {
        ...
        let (id, purpose) = row?;
        references.prompt_purposes.insert(
            id,
            crate::catalog::prompt_adapter::purpose_from_name(&purpose)?,
        );
    }
...
mod tests {
    ...
    fn a_companion_file_writes_its_pool_soul_and_relationships_with_the_character() {
        ...
        assert_eq!(facts, 1);
        drop(connection);
        let graph = crate::backup::restore_writer::tests::assert_backup_round_trip(&database);
