    ...
    I: InferencePort + ?Sized,
{
    ...
    pub async fn resume_after_restart(
        ...
        for job in self.queued_memory_jobs()? {
            let interrupted = crate::jobs::job_recovery::interrupted_runs(self.repository, job.id)
            ...
            if let Err(error) =
                crate::jobs::job_recovery::fail_interrupted_job(self.repository, &job, clock.now())
            {
    ...
    fn built_in_prompt(
        ...
        crate::generation::built_in_prompts::active_built_in_prompt(self.repository, id)
            .map_err(|_| storage())?
            .ok_or(CompanionMemoryHostError::RuntimeInputs(
...
fn clock_error(
    error: crate::companion::companion_clock::CompanionClockError,
) -> CompanionMemoryHostError {
    match error {
        crate::companion::companion_clock::CompanionClockError::Character(error) => {
            CompanionMemoryHostError::Character(error)
        }
        crate::companion::companion_clock::CompanionClockError::MissingCharacter => {
            CompanionMemoryHostError::Conversation(ConversationRepositoryError::NotFound)
        }
        crate::companion::companion_clock::CompanionClockError::Companion(error) => {
