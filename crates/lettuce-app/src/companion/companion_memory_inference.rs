...
impl MemoryFallbackKind {
    ...
    fn accepts(self, outcome: &InferenceOutcome) -> bool {
        match self {
            ...
            Self::Repairs => {
                !crate::companion::companion_memory_repair::repaired_categories(outcome).is_empty()
            }
        }
    }
}
        ...
        + ?Sized,
{
    ...
    let fallback_prompt = crate::generation::runtime_text::RuntimeText::load(
        repository,
        crate::BuiltInPromptId::MemoryRuntime,
    )
...
fn build_first_request(
    ...
    let memory_lines = memory
        ...
        .map(|item| {
            crate::memory::memory_tool_result::memory_id_line(text, item.short_id, &item.text)
        })
    ...
    if run.time_awareness_enabled {
        crate::companion::companion_memory_summary::insert_time_values(&mut values, now);
    }
...
mod tests {
    ...
    fn dynamic_memory_tool_request() -> lettuce_conversations::ToolRequest {
        crate::companion::companion_memory_run::test_memory_tool_request(false, false)
    }
    use lettuce_models::{
    ...
    fn request_copies_legacy_companion_prompt_and_runtime_input() {
        ...
        let request = build_first_request(
            ...
            &attempt,
            &prompt,
            &crate::generation::runtime_text::RuntimeText::from_seed(
                crate::BuiltInPromptId::MemoryRuntime,
            ),
            "Prior summary.",
            &policy,
        ...
        time_aware_run.tool_request =
            crate::companion::companion_memory_run::test_memory_tool_request(true, true);
        let sources = [
            MaterializedSource {
