    ...
    Ok(messages
        ...
        .map(|(index, item)| {
            ...
            let content = if time_awareness_enabled {
                format!(
                    "{} {}",
                    crate::companion::companion_memory_inference::format_message_timestamp(
...
fn format_selected_memories(
    ...
    none: &str,
    fragment: &Fragment<'_>,
    memory_text: Option<&crate::generation::runtime_text::RuntimeText>,
) -> Result<String, LorebookEntryPreparationError> {
    ...
    let lines = matches
        ...
        .map(|(index, memory)| {
            let (text, observed) = if time_awareness_enabled {
                let line = crate::memory::memory_prompt::memory_prompt_line(memory, effective_now);
                let observed = line
                    .observed
                    .as_ref()
                    .map(|observation| {
                        crate::memory::memory_prompt::render_observation(
...
mod tests {
    ...
    fn selected_memories_keep_the_legacy_numbered_observed_format() {
        ...
        let memory_text = crate::generation::runtime_text::RuntimeText::from_seed(
            crate::BuiltInPromptId::MemoryRuntime,
        );
        ...
        .expect("time aware");
        let observed = crate::memory::memory_prompt::memory_prompt_line(&memories[0], now)
            .observed
            .expect("observed")
            .local_time;
        assert_eq!(
