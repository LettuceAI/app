    ...
    I: InferencePort + ?Sized,
{
    pub async fn run(
        ...
        if attempts.is_empty() {
            ...
            match crate::jobs::job_inference_usage::run_job_inference(
                self.repository,
                self.inference,
                run.job_id,
                request,
            ...
            .await
            {
                ...
                Err(crate::jobs::job_inference_usage::JobInferenceError::Evidence) => {
                    return Err(LorebookEntryExecutionError::Inference(
        ...
        let outcome = crate::jobs::job_inference_usage::run_job_inference(
            self.repository,
            self.inference,
            run.job_id,
            request,
