    ...
    T: serde::de::DeserializeOwned,
{
    ...
    for row in rows {
        ...
        if let Some(value) = tags {
            if let Ok(parsed) = serde_json::from_str::<Vec<String>>(&value) {
                obj.insert("tags".into(), serde_json::json!(parsed));
            }
        }
...
fn upsert_persona_value(app: &tauri::AppHandle, p: &JsonValue) -> Result<JsonValue, String> {
    ...
    let created_at = tracked_write_string(&conn, |tx| {
        ...
        tx.execute(
            r#"INSERT INTO personas (id, title, description, nickname, avatar_path, avatar_crop_x, avatar_crop_y, avatar_crop_scale, design_description, design_reference_image_ids, active_lorebook_ids, is_default, created_at, updated_at, tags)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
pub fn persona_default_get(app: tauri::AppHandle) -> Result<Option<String>, String> {
    let conn = open_db(&app)?;
    let row = conn.query_row("SELECT id, title, description, nickname, avatar_path, avatar_crop_x, avatar_crop_y, avatar_crop_scale, design_description, design_reference_image_ids, COALESCE(active_lorebook_ids, '[]'), is_default, created_at, updated_at, tags FROM personas WHERE is_default = 1 LIMIT 1", [], |r| Ok((
        r.get::<_, String>(0)?, r.get::<_, String>(1)?, r.get::<_, String>(2)?, r.get::<_, Option<String>>(3)?, r.get::<_, Option<String>>(4)?, r.get::<_, Option<f64>>(5)?, r.get::<_, Option<f64>>(6)?, r.get::<_, Option<f64>>(7)?, r.get::<_, Option<String>>(8)?, r.get::<_, Option<String>>(9)?, r.get::<_, String>(10)?, r.get::<_, i64>(11)?, r.get::<_, i64>(12)?, r.get::<_, i64>(13)?, r.get::<_, Option<String>>(14)?
